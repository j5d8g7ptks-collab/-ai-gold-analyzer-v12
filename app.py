...
st.caption("H1 الاتجاه • M15 السياق • M1 نقطة الدخول")
# --- Professional compact mobile-first UI (display only) ---
st.markdown("""
<style>
.block-container{padding-top:1rem;padding-bottom:1rem;max-width:1100px;}
[data-testid="stMetric"]{padding:.55rem .7rem;border:1px solid rgba(128,128,128,.22);border-radius:14px;}
.compact-card{padding:16px 18px;border:1px solid rgba(128,128,128,.22);border-radius:18px;margin:8px 0 14px;}
.signal-big{font-size:2.2rem;font-weight:800;line-height:1.1;margin:2px 0;}
.signal-sub{font-size:.95rem;opacity:.78;}
@media (max-width:700px){.block-container{padding-left:.75rem;padding-right:.75rem;}h1{font-size:1.65rem!important;}h2{font-size:1.2rem!important;}.signal-big{font-size:2rem;}[data-testid="stMetricValue"]{font-size:1.35rem;}}
</style>
...
try:
    ...
    # DISPLAY ONLY: compact decision-first layout. Trading/model logic is unchanged.
    st.markdown("### 🎯 القرار الآن")
    if signal == "WAIT":
        st.markdown("<div class='compact-card'><div class='signal-big'>🟡 WAIT</div><div class='signal-sub'>لا توجد فرصة واضحة حالياً</div></div>", unsafe_allow_html=True)
        a,b,c = st.columns(3)
        a.metric("الثقة", f"{max(p,1-p)*100:.1f}%")
        b.metric("الاتجاه العام", h1_direction)
        c.metric("قوة الحركة", f"ADX {float(last['adx']):.1f}")
        st.info("انتظار تأكيد أقوى على M1 قبل إعطاء دخول.")
    else:
        icon = "🟢" if signal == "BUY" else "🔴"
        st.markdown(f"<div class='compact-card'><div class='signal-big'>{icon} {signal}</div><div class='signal-sub'>فرصة واضحة • دخول M1</div></div>", unsafe_allow_html=True)
        a,b,c = st.columns(3)
        a.metric("الثقة", f"{max(p,1-p)*100:.1f}%")
        b.metric("الدخول M1", f"{entry:.2f}")
        c.metric("وقت الإشارة", last.name.strftime("%H:%M:%S UTC"))
        x1,x2,x3 = st.columns(3)
        x1.metric("Stop Loss", f"{sl:.2f}")
        x2.metric("Take Profit", f"{tp:.2f}")
        x3.metric("ADX M1", f"{float(last['adx']):.1f}")

    with st.expander("📊 صورة السوق", expanded=True):
        e,f,g = st.columns(3)
        e.metric("H1", h1_direction)
        f.metric("M15", m15_direction)
        g.metric("M1", m1_direction)
    ...
    st.subheader("📈 الأداء السابق")
    q1,q2,q3,q4 = st.columns(4)
    q1.metric("الإشارات", str(total))
    q2.metric("صحيحة", str(wins))
    q3.metric("خاطئة", str(losses))
    q4.metric("Win Rate", f"{winrate:.1f}%")

    with st.expander("🧾 آخر الإشارات", expanded=False):
