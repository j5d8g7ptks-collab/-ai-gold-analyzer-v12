...
st.set_page_config(page_title="AI GOLD ANALYZER V14.3", page_icon="🧠", layout="wide")
...
st.markdown('<div class="hero"><h1>🧠 AI GOLD ANALYZER V14.3</h1><p>MTF + Calibrated ML + Meta-Label + Regime + Cost-aware OOS audit — research / paper trading</p></div>', unsafe_allow_html=True)
...
def load_bars(interval, limit=1000, min_bars=0):
    target = min(int(limit), 1000)
    if min_bars:
        target = max(target, int(min_bars))

    all_rows = []
    cursor_to = None

    # Some feed deployments may return fewer bars than requested.
    # Walk backwards in time so the model still receives a useful history.
    for _ in range(6):
        ...
        r = requests.get(
            ...
            params=params,
            headers={"User-Agent": "AI-Gold-Analyzer-V14.1/1.0"},
            timeout=20,
        )
        r.raise_for_status()
        payload = r.json()
        ...
        parsed_times = [
            pd.to_datetime(x["time"], utc=True, errors="coerce")
            for x in all_rows[-1000:]
            if x.get("time") is not None
        ]
        parsed_times = [x for x in parsed_times if not pd.isna(x)]
        if not parsed_times:
            break

        oldest = min(parsed_times)
        # ISO UTC value accepted by BiQuote's `to` parameter.
        cursor_to = (oldest - pd.Timedelta(minutes=1)).strftime("%Y-%m-%dT%H:%M:%SZ")

        # Prevent an endless loop if the provider repeats the same page.
        if len(bars) < 2:
            break
    ...
    if len(df) < 120:
        raise ValueError(f"تم استلام {len(df)} شمعة مغلقة فقط لـ {interval}.")

    # Keep the newest requested bars after pagination.
    df = df.tail(target)
...
def enrich_mtf(m1, m15, h1):
    x = m1.copy()
    x["h1_trend"] = ema_trend(h1).reindex(x.index, method="ffill")
...
try:
    m1 = load_bars("1m", 1000, min_bars=500)
    m15 = load_bars("15m", 1000, min_bars=200)
    h1_bars = load_bars("1h", 1000, min_bars=120)
    ...
    if len(clean) < MIN_TRAIN + HORIZON_M1 + 30:
        raise ValueError(
            f"البيانات الصالحة للتدريب قليلة: {len(clean)} صف فقط "
            f"(المطلوب {MIN_TRAIN + HORIZON_M1 + 30})."
        )

    sample_limited = len(clean) < 320
    ...
    # Small-sample adaptive mode:
    # never crash the app just because OOF Meta-Label samples are below
    # the preferred institutional target. If enough labeled samples exist,
    # train the meta model; otherwise use a conservative fallback.
    # =========================
    meta_base = clean.iloc[:-HORIZON_M1].copy()
    if len(meta_base) < 100:
        raise ValueError("التاريخ المتاح أقل من الحد الأدنى لبناء Meta-Label.")

    oof = np.full(len(meta_base), np.nan)
    # With small histories, accepting folds with >=60 training rows
    # produces useful OOF coverage instead of discarding most samples.
    ...
    meta_df = meta_training_frame(meta_base, oof_series).dropna()
    meta_cols = [c for c in meta_df.columns if c != "label"]

    meta_model = None
    meta_mode = "fallback"
    if len(meta_df) >= 60 and meta_df["label"].nunique() >= 2:
        meta_model = RandomForestClassifier(
            n_estimators=160, max_depth=5, min_samples_leaf=5,
        ...
        # Conservative abstention fallback: no Meta-Label evidence means
        # do not pretend that the meta model is confident.
        meta_p = 0.50

    meta_floor = 0.60 if meta_mode == "trained" else 0.66

    effective_threshold = max(float(user_threshold), 0.72) if sample_limited else float(user_threshold)
    effective_meta_threshold = max(float(meta_threshold), meta_floor) if sample_limited else max(float(meta_threshold), meta_floor)
    signal, reason = decide(last, p_live, meta_p, effective_threshold, effective_meta_threshold)
    if sample_limited and signal != "WAIT":
        reason += " العينة الحالية محدودة، لذلك رُفع حد القرار تلقائياً."
    ...
    if meta_mode == "fallback":
        st.warning("⚠️ Meta-Label يعمل بوضع محافظ مؤقتاً لأن عدد عينات WIN/LOSS غير كافٍ لتدريبه بثقة. لذلك لا يسمح النظام بإشارة سهلة، ويُفضّل WAIT حتى تتوفر عينة أكبر.")
    if sample_limited:
        st.warning(f"⚠️ تاريخ M1 المتاح حالياً محدود ({len(clean)} صف صالح). النظام يعمل بوضع محافظ ويرفع حدود الإشارة تلقائياً بدلاً من اختلاق بيانات إضافية.")
