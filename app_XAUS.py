...
def load_gold(interval_value, period_value):
    ...
    points = payload.get("data") or payload.get("prices") or []
    if not points:
        raise ValueError("XAUS returned no intraday XAU/USD data.")
    ...
    for item in points:
        ...
        if ts is None or price is None:
            continue
        try:
            rows.append((pd.to_datetime(ts, utc=True), float(price)))
...
# XAUS note: the feed is a reference market price, not a broker execution quote.
# Keep the dashboard in research/paper-trading mode until broker-matched data is added.
# V12 XAUS source integration
# V12 XAUS source integration
# V12 XAUS source integration
# V12 XAUS source integration
# V12 XAUS source integration
