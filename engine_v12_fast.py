...
def train_model(df, features):
    ...
    cut = max(1, min(len(d)-1, int(len(d)*0.70)))
    train = d.iloc[:cut]
    test = d.iloc[cut:]

    # Faster model for Streamlit/iPhone use.
...
def predict_latest(model, calibration, df, features):
    ...
    row = valid.iloc[-1]
    p = float(_prob_up(model, row[features].to_frame().T)[0])
    ...
    state = (
        "Bullish" if row["ema20"] > row["ema50"] > row["ema200"]
        else "Bearish" if row["ema20"] < row["ema50"] < row["ema200"]
        else "Mixed"
    )
    return {
...
def backtest(df, model, calibration, features, rr=2.0, horizon=40):
    ...
    end = max(start, len(d)-horizon)
    if end <= start:
        return {"win_rate":0.0,"trades":0,"profit_factor":0.0,"max_dd":0.0,"expectancy":0.0}, pd.DataFrame()

    # Predict all rows in one batch instead of thousands of model calls.
    segment = d.iloc[start:end]
    probs = _prob_up(model, segment[features])

    # Keep the backtest responsive on cloud.
    idxs = np.flatnonzero((probs >= 0.60) | (probs <= 0.40))
    if len(idxs) > 800:
        idxs = idxs[-800:]
    ...
    for k in idxs:
        ...
        rows.append({
            ...
            "signal": sig,
            "confidence": max(p,1-p)*100,
            "entry": entry, "sl": sl, "tp": tp, "r": result
        })

    trades = pd.DataFrame(rows)
    if trades.empty:
        return {"win_rate":0.0,"trades":0,"profit_factor":0.0,"max_dd":0.0,"expectancy":0.0}, trades
